...
export default class Asset {
  static async transfer(privateKey, to, amount, assetName = { GLOBAL_SYMBOL }) {
    try {
      ...
      if (!res.ok || result.status !== 'success') return { tranID: '', result: false }

      const signature = bytes.byteArray2hexStr(
        new Uint8Array(
          crypto.signBytes(privateKey, code.hexStr2byteArray(result.data.tran_raw_hex))
        )
      )
      const status = await transaction.broadcast(
        result.data.tran_hex,
        result.data.tran_raw_hex,
        signature
      )
